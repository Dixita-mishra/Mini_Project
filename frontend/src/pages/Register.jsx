...
import { MdPersonOutline, MdLockOutline, MdOutlineShield, MdEmail, MdPhone, MdLocationCity } from 'react-icons/md';
...
const Register = () => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        display: 'flex', width: '100%', maxWidth: '850px', minHeight: '520px',
        ...
        <div style={{
          flex: 1.1, padding: '60px 40px', display: 'flex', flexDirection: 'column',
          justifyContent: 'center', background: 'linear-gradient(135deg, #81c7f5 0%, #90caf9 100%)',
        ...
        <div style={{
          flex: 0.9, padding: '40px 40px', display: 'flex', flexDirection: 'column',
          justifyContent: 'center', position: 'relative'
        }}>
          
          <div style={{ marginBottom: '20px' }}>
            <h2 style={{ fontSize: '28px', fontWeight: 800, color: '#1f2937', marginBottom: '6px' }}>
              {new URLSearchParams(window.location.search).get('role') === 'admin' ? 'Admin Registration' : 'Create Account'}
            </h2>
            <p style={{ fontSize: '13px', color: '#4b5563' }}>Enter your details to register</p>
          </div>
          ...
          <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ position: 'relative' }}>
              <input type="text" name="name" placeholder="Full Name" value={formData.name} onChange={handleChange} required
                style={{ width: '100%', padding: '12px 16px', paddingRight: '40px', background: 'rgba(255, 255, 255, 0.65)', border: '1px solid rgba(255, 255, 255, 0.8)', borderRadius: '10px', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }}
              />
              <MdPersonOutline style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', color: '#4b5563', fontSize: 18 }} />
            </div>

            <div style={{ position: 'relative' }}>
              <input type="email" name="username" placeholder="Email Address" value={formData.username} onChange={handleChange} required
                style={{ width: '100%', padding: '12px 16px', paddingRight: '40px', background: 'rgba(255, 255, 255, 0.65)', border: '1px solid rgba(255, 255, 255, 0.8)', borderRadius: '10px', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }}
              />
            ...
            <button type="submit" disabled={loading} style={{
              marginTop: '10px', padding: '14px', background: '#11b2ac', color: '#fff', border: 'none',
              borderRadius: '10px', fontSize: '13px', fontWeight: 700, letterSpacing: '0.5px', cursor: 'pointer',
            ...
            <div style={{ textAlign: 'center', marginTop: '10px', fontSize: '13px' }}>
               ...
               <Link to="/login" style={{ color: '#11b2ac', fontWeight: 600, textDecoration: 'none' }}>Login here</Link>
            </div>
