              ...
              {documentFields.map((doc) => (
                ...
                >
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{doc.label}</span>
                    {form.documents[doc.key] && (
                      <span style={{ fontSize: 12, color: 'var(--blue-light)', marginTop: 4 }}>
...
const MyClaims = () => {
  ...
  useEffect(() => {
    ...
    const defaultPolicies = [
      { id: 1, name: 'Health Insurance Plus' },
      { id: 2, name: 'Family Health Cover' },
      { id: 3, name: 'Accident Protection Plan' },
      { id: 4, name: 'Premium Care Policy' }
    ];
            ...
            <div style={{ textAlign: 'right' }}>
              <div
                style={{
                  fontSize: 22,
                  fontWeight: 800,
                  color: c.status === 'approved' ? 'var(--emerald-light)' : 'var(--text-primary)'
                }}
              >
                {c.amount}
              </div>
      ...
      {claims.length === 0 && (
        <div className="chart-card">
          <div className="empty-state">
            ...
            <button className="btn btn-primary" style={{ margin: '16px auto' }} onClick={() => setModal(true)}>
