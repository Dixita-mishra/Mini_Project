...
const normalUserNav = [
  { section: 'MY ACCOUNT', items: [
    ...
    { icon: <MdCreditCard />, label: 'Payments', path: '/user/payments' },
  ]},
  { section: 'CLAIMS & SUPPORT', items: [
    { icon: <MdAssignment />, label: 'My Claims', path: '/user/claims' },
    { icon: <MdInfoOutline />, label: 'Claim Status', path: '/user/claim-status' },
    { icon: <MdAccountCircle />, label: 'Profile', path: '/user/profile' },
    { icon: <MdHeadsetMic />, label: 'Support', path: '/user/support' },
  ]},
];
...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const handleSwitch = (p) => {
    switchPortal(p);
    navigate(p === 'admin' ? '/admin/dashboard' : p === 'agent' ? '/agent/dashboard' : '/user/dashboard');
  };
  ...
  const avatarText = tokenPayload?.name
    ? tokenPayload.name.charAt(0).toUpperCase()
    : portal === 'admin' ? 'A' : portal === 'agent' ? 'G' : 'U';

  const profileName = tokenPayload?.name || tokenPayload?.email || (portal === 'admin' ? 'Admin Profile' : portal === 'agent' ? 'Agent Profile' : 'User Profile');
    ...
    <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
      ...
      <div className="sidebar-portal-toggle">
        <div className="portal-toggle-btns">
          <button className={`portal-btn ${portal === 'admin' ? 'active' : ''}`} onClick={() => handleSwitch('admin')}>Admin</button>
          <button className={`portal-btn ${portal === 'agent' ? 'active' : ''}`} onClick={() => handleSwitch('agent')}>Agent</button>
          <button className={`portal-btn ${portal === 'user' ? 'active' : ''}`} onClick={() => handleSwitch('user')}>User</button>
        </div>
      </div>
        ...
        {navItems.map((section) => (
          <div key={section.section}>
            ...
            {section.items.map((item) => (
              <div key={item.path} className={`nav-item ${location.pathname === item.path ? 'active' : ''}`} onClick={() => handleNav(item.path)}>
      ...
      <div className="sidebar-footer" style={{ borderTop: 'none', borderBottom: '1px solid var(--border-color)', margin: '0 1rem 1rem 1rem', padding: '1rem 0' }}>
        <div className="sidebar-user-card" style={{ padding: '0.5rem', cursor: 'pointer', transition: 'background 0.2s', borderRadius: '8px' }} onClick={() => setShowUserDetails(!showUserDetails)} onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--bg-secondary)')} onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}>
        ...
        {showUserDetails && (
          <div style={{ marginTop: '10px', padding: '12px', background: 'var(--bg-secondary)', borderRadius: '8px', border: '1px solid var(--border-color)', fontSize: '13px' }}>
            ...
            <button onClick={handleLogout} style={{ width: '100%', padding: '8px', background: '#ef4444', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontWeight: 600 }}>
              <MdLogout /> Logout
            </button>
          </div>
        )}
